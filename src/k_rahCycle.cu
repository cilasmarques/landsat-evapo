...
__global__ void d0_kernel(float *pai_d, float *d0_d, float CD1, float HGHT)
{
    ...
    if (pos < width_d * height_d) {
        ...
        d0_d[pos] = HGHT * ((1.0f - (1.0f / cd1_pai_root)) + (exp(-cd1_pai_root) / cd1_pai_root));
    }
}
...
__global__ void ustar_kernel_STEEP(float *zom_d, float *d0_d, float *ustar_d, float u10)
{
    ...
    if (pos < width_d * height_d) {
        ustar_d[pos] = (u10 * VON_KARMAN) / log((10.0f - d0_d[pos]) / zom_d[pos]);
    }
}
...
__global__ void ustar_kernel_ASEBAL(float *zom_d, float *ustar_d, float u200)
{
    ...
    if (pos < width_d * height_d) {
        ustar_d[pos] = (u200 * VON_KARMAN) / log(200.0f / zom_d[pos]);
    }
}
...
__global__ void zom_kernel_STEEP(float *d0_d, float *pai_d, float *zom_d, float A_ZOM, float B_ZOM)
{
    ...
    if (pos < width_d * height_d) {
        float gama = pow((CD + CR * (pai_d[pos] / 2.0f)), -0.5f);
        if (gama < 3.3f)
            gama = 3.3f;
...
__global__ void kb_kernel(float *zom_d, float *ustar_d, float *pai_d, float *kb1_d, float *ndvi_d, float ndvi_max, float ndvi_min)
{
    ...
    if (pos < width_d * height_d) {
        float fc = 1.0f - pow((ndvi_d[pos] - ndvi_max) / (ndvi_min - ndvi_max), 0.4631f);
        float fs = 1.0f - fc;

        float Re = (ustar_d[pos] * 0.009f) / visc;
        float Ct = pow(pr, -0.667f) * pow(Re, -0.5f);
        float ratio = c1 - c2 * (exp(cd * -c3 * pai_d[pos]));
        float nec = (cd * pai_d[pos]) / (ratio * ratio * 2.0f);
        float kbs = 2.46f * pow(Re, 0.25f) - 2.0f;

        float kb1_fst_part = (cd * VON_KARMAN) / (4.0f * ct * ratio * (1.0f - exp(nec * -0.5f)));
        float kb1_sec_part = pow(fc, 2.0f) + (VON_KARMAN * ratio * (zom_d[pos] / HGHT) / Ct);
        float kb1_trd_part = pow(fc, 2.0f) * pow(fs, 2.0f) + kbs * pow(fs, 2.0f);
        float kb_ini = kb1_fst_part * kb1_sec_part * kb1_trd_part;

	float SF = sf_c + (1.0f / (1.0f + exp(-sf_d) - sf_e * soil_moisture_day_rel));
...
__global__ void aerodynamic_resistance_kernel_STEEP(float *zom_d, float *d0_d, float *ustar_d, float *kb1_d, float *rah_d)
{
    ...
    if (pos < width_d * height_d) {
        ...
        float rah_sec_part = log((10.0f - d0_d[pos]) / zom_d[pos]);
...
__global__ void aerodynamic_resistance_kernel_ASEBAL(float *ustar_d, float *rah_d)
{
    ...
    if (pos < width_d * height_d) {
        rah_d[pos] = log(2.0f / 0.1f) / (ustar_d[pos] * VON_KARMAN);
    }
}
...
__global__ void rah_correction_cycle_STEEP(float *surface_temperature_d, float *d0_d, float *kb1_d, float *zom_d, float *ustar_d, float *rah_d, float *H_d)
{
    ...
    if (pos < width_d * height_d) {
        ...
        float L = -1.0f * ((RHO * SPECIFIC_HEAT_AIR * pow(ustar_d[pos], 3.0f) * surface_temperature_d[pos]) / (VON_KARMAN * GRAVITY * H_d[pos]));

        float y2 = pow((1.0f - (16.0f * (10.0f - d0_d[pos])) / L), 0.25f);
        float x200 = pow((1.0f - (16.0f * (10.0f - d0_d[pos])) / L), 0.25f);
        ...
        if (!isnan(L) && L > 0) {
            ...
            psi2 = 2.0f * log((1.0f + y2 * y2) / 2.0f);
            psi200 = 2.0f * log((1.0f + x200) / 2.0f) + log((1.0f + x200 * x200) / 2.0f) - 2.0f * atan(x200) + 0.5f * PI;
        }

        ustar_d[pos] = (VON_KARMAN * ustar_d[pos]) / (log((10.0f - d0_d[pos]) / zom_d[pos]) - psi200);

        float rah_fst_part = 1.0f / (ustar_d[pos] * VON_KARMAN);
        float rah_sec_part = log((10.0f - d0_d[pos]) / zom_d[pos]) - psi2;
        float rah_trd_part = rah_fst_part * kb1_d[pos];
        rah_d[pos] = (rah_fst_part * rah_sec_part) + rah_trd_part;
    }
}
...
__global__ void rah_correction_cycle_ASEBAL(float *surface_temperature_d, float *zom_d, float *ustar_d, float *rah_d, float *H_d, float u200)
{
    ...
    if (pos < width_d * height_d) {
        ...
        float x1 = pow((1.0f - (16.0f * 0.1f) / L), 0.25f);
        float x2 = pow((1.0f - (16.0f * 2.0f) / L), 0.25f);
        float x200 = pow((1.0f - (16.0f * 200.0f) / L), 0.25f);
        ...
        if (!isnan(L) && L > 0) {
            ...
            psi1 = 2.0f * log((1.0f + x1 * x1) / 2.0f);
            psi2 = 2.0f * log((1.0f + x2 * x2) / 2.0f);
            psi200 = 2.0f * log((1.0f + x200) / 2.0f) + log((1.0f + x200 * x200) / 2.0f) - 2.0f * atan(x200) + 0.5f * PI;
        }

        ustar_d[pos] = (VON_KARMAN * u200) / (log(200.0f / zom_d[pos]) - psi200);
        rah_d[pos] = (log(2.0f / 0.1) - psi2 + psi1) / (ustar_d[pos] * VON_KARMAN);
    }
}
