...
string getEndmembersSTEEP(Products products, vector<Endmember>& hotCandidates, vector<Endmember>& coldCandidates) {
    string result = "";
    float general_time;
    ...
    return "SERIAL,PIXEL_FILTER," + std::to_string(general_time) + "," + std::to_string(initial_time) + "," + std::to_string(final_time) + "\n";
}

string getEndmembersASEBAL(Products products, vector<Endmember>& hotCandidates, vector<Endmember>& coldCandidates)
{
    string result = "";
    float general_time;
    int64_t initial_time, final_time;
...
string Products::select_endmembers(Products products)
{
    ...
    try
    {
        ...
        if (model_method == 0)
            result += getEndmembersSTEEP(products, hotCandidates, coldCandidates);
        else if (model_method == 1) 
            result += getEndmembersASEBAL(products, hotCandidates, coldCandidates);
