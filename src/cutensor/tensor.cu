...
void Tensor::createPlanWork(cutensorOperationDescriptor_t desc)
{
  ...
  HANDLE_CUTENSOR_ERROR(cutensorCreatePlanPreference(handle, &planPref, algo, CUTENSOR_JIT_MODE_NONE));

  // Query workspace estimate
  uint64_t workspaceSizeEstimate = 0;
  const cutensorWorksizePreference_t workspacePref = CUTENSOR_WORKSPACE_DEFAULT;
  HANDLE_CUTENSOR_ERROR(cutensorEstimateWorkspaceSize(handle, desc, planPref, workspacePref, &workspaceSizeEstimate));

  // Create Plan
  HANDLE_CUTENSOR_ERROR(cutensorCreatePlan(handle, &plan, desc, planPref, workspaceSizeEstimate));
...
void Tensor::createNormalTrinity(int height_band, int width_band)
{
  ...
  HANDLE_CUTENSOR_ERROR(cutensorCreateTensorDescriptor(this->handle, &descB, dim_num, axis_dim.data(), NULL, CUTENSOR_R_32F, kAlignment));
  HANDLE_CUTENSOR_ERROR(cutensorCreateTensorDescriptor(this->handle, &descC, dim_num, axis_dim.data(), NULL, CUTENSOR_R_32F, kAlignment));
  HANDLE_CUTENSOR_ERROR(cutensorCreateTensorDescriptor(this->handle, &descD, dim_num, axis_dim.data(), NULL, CUTENSOR_R_32F, kAlignment));
