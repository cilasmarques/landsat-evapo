...
__host__ __device__ Candidate::Candidate()
{
    this->ndvi = 0;
    this->temperature = 0;
...
__host__ __device__ Candidate::Candidate(float ndvi, float temperature, float net_radiation, float soil_heat_flux, float ho, int line, int col)
{
    this->ndvi = ndvi;
