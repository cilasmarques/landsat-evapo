...
string Products::select_endmembers(int method)
{
  ...
  if (method == 0)
  { // STEEP
    result += getEndmembersSTEEP(ndvi_d, surface_temperature_d, albedo_d, net_radiation_d, soil_heat_d,
                                 blocks_num, threads_num, d_hotCandidates, d_coldCandidates, height_band, width_band);
  }
  else if (method == 1)
  { // ASEBAL
    result += getEndmembersASEBAL(ndvi_d, surface_temperature_d, albedo_d, net_radiation_d, soil_heat_d,
                                  blocks_num, threads_num, d_hotCandidates, d_coldCandidates, height_band, width_band);
  }
  cudaEventRecord(stop);
